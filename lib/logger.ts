...
if (!isEdgeRuntime) {
  // Console output (always enabled in development, or in production serverless)
  ...
  // File logging only in development (not in serverless production)
  if (ENV.isDevelopment) {
    // Only import fs and path when actually needed
    const fs = require('fs');
...
export function createYouTubeApiLogger(name: string) {
  // In Edge Runtime or production serverless, just use the base logger
  if (isEdgeRuntime || !logsDir || ENV.isProduction) {
